...
def results(experiment_id: str, db: Session = Depends(get_db)):
    """Run the full analysis: SRM guardrail, frequentist test(s), Bayesian view, and a
    single ship / hold recommendation."""
    experiment = _require_experiment(experiment_id, db)
    try:
        return service.analyze(db, experiment)
...
def report(experiment_id: str, db: Session = Depends(get_db)):
    ...
    experiment = _require_experiment(experiment_id, db)
    try:
        return service.build_report(db, experiment).to_dict()
    except ValueError as exc:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc))
...
def charts(experiment_id: str, db: Session = Depends(get_db)):
    """Plotly figures for the experiment (JSON), ready for the dashboard to render."""
    experiment = _require_experiment(experiment_id, db)
    try:
        return service.build_charts(db, experiment)
    except ValueError as exc:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, str(exc))
